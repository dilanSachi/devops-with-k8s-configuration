...
This repository contains the configurations related to the deployments of the todo-app project in the DevOps with Kubernetes course offered by University of Helsinki.
...
```
#### Build & run locally
* Execute `./build.sh` inside the `ex410` directory in `https://github.com/dilanSachi/devops-with-k8s-exercises`. This will build the todo app, broadcaster app and todo backend java projects, 
...
#### Deployment pipeline
* Whenever a change is pushed to the `ex410/` directory in `https://github.com/dilanSachi/devops-with-k8s-exercises`, the GitHub actions will catch it and will push the images to docker hub and update the image versions in the `kustomization.yaml` files.
